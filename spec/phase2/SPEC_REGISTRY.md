...
| **FC-REG6** | Registry storage path | **`<storage.root>/registries/`** (under existing config). Default resolves to `~/.eden-skills/skills/registries/`. | Consolidates all eden-skills data under `storage.root`. If the user moves `storage.root`, registries move with it. Matches the default established in Phase 1/2.5. |
