...
| **ARC-104** | Builder | **P1** | The `DockerAdapter` MUST support `cp` (copy) injection strategy as the primary reliable method. | File appears in container after `eden install` even without shared volumes. |
...
- **Trade-off:** Updates are not real-time (no symlink magic). `eden update`
...
1. `eden install --target local` produces identical behavior to Phase 1 `apply`.
2. `eden install --target docker:test-container` copies skill files into a
