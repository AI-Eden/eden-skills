...
| CMD-SAFETY-002 | `SPEC_COMMANDS.md` 3.5.3 | mixed-skill no-exec behavior: skip verify only for no-exec skills and exclude no-exec conflicts from strict conflict exits | `crates/eden-skills-core/src/verify.rs` + `crates/eden-skills-cli/src/commands.rs` | `crates/eden-skills-cli/tests/safety_gate.rs` (`apply_mixed_skills_skips_verify_only_for_no_exec_skill`, `apply_mixed_skills_still_verifies_non_no_exec_skill`, `apply_strict_ignores_conflicts_for_no_exec_skill`, `repair_strict_ignores_conflicts_for_no_exec_skill`) | implemented |
...
| CMD-STRICT-001 | `SPEC_COMMANDS.md` 3.4.1 | strict conflict exit precedence is consistent across `apply`/`repair` and takes precedence over post-mutation verification failures when source sync succeeds | `crates/eden-skills-cli/src/commands.rs` | `crates/eden-skills-cli/tests/exit_code_matrix.rs` (`apply_strict_conflict_takes_precedence_over_verify_failure`, `repair_strict_conflict_takes_precedence_over_verify_failure`) | implemented |
...
| TM-STRICT-003 | `SPEC_TEST_MATRIX.md` 8 | `apply --strict` returns exit code `3` when conflict and verify-failure conditions coexist | `crates/eden-skills-cli/tests/exit_code_matrix.rs` (`apply_strict_conflict_takes_precedence_over_verify_failure`) | covered |
| TM-STRICT-004 | `SPEC_TEST_MATRIX.md` 8 | `repair --strict` returns exit code `3` when conflict and verify-failure conditions coexist | `crates/eden-skills-cli/tests/exit_code_matrix.rs` (`repair_strict_conflict_takes_precedence_over_verify_failure`) | covered |

## 7. Incremental Safety Interaction Coverage

| SCENARIO_ID | Source | Scenario | Automated Test | Status |
|---|---|---|---|---|
| TM-SAFETY-001 | `SPEC_TEST_MATRIX.md` 5 | mixed-skill configs skip verify only for `no_exec_metadata_only=true` skills | `crates/eden-skills-cli/tests/safety_gate.rs` (`apply_mixed_skills_skips_verify_only_for_no_exec_skill`) | covered |
| TM-SAFETY-002 | `SPEC_TEST_MATRIX.md` 5 | mixed-skill configs still verify non-no-exec skills deterministically | `crates/eden-skills-cli/tests/safety_gate.rs` (`apply_mixed_skills_still_verifies_non_no_exec_skill`) | covered |
| TM-SAFETY-003 | `SPEC_TEST_MATRIX.md` 5 | strict conflict exits ignore conflicts from no-exec skills (`apply`) | `crates/eden-skills-cli/tests/safety_gate.rs` (`apply_strict_ignores_conflicts_for_no_exec_skill`) | covered |
| TM-SAFETY-004 | `SPEC_TEST_MATRIX.md` 5 | strict conflict exits ignore conflicts from no-exec skills (`repair`) | `crates/eden-skills-cli/tests/safety_gate.rs` (`repair_strict_ignores_conflicts_for_no_exec_skill`) | covered |
